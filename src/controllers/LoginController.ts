...
router.post(
  ...
  async (req, res) => {
    ...
    res.cookie(COOKIE_REFRESH_TOKEN_KEY, refreshToken, { secure: false, httpOnly: true, maxAge: MS_IN_DAY, sameSite: 'lax' });
...
router.post('/logout', async (req, res) => {
  ...
  if (!user) {
    res.clearCookie(COOKIE_REFRESH_TOKEN_KEY, { secure: false, httpOnly: true, sameSite: 'lax' });
